...
from database.connections_mdb import active_connection, all_connections, delete_connection, if_active, make_active, \
    make_inactive
from info import ADMINS, AUTH_CHANNEL, AUTH_USERS, SUPPORT_CHAT_ID, CUSTOM_FILE_CAPTION, REQ_CHANNEL, MSG_ALRT, PICS, AUTH_GROUPS, P_TTI_SHOW_OFF, GRP_LNK, CHNL_LNK, NOR_IMG, LOG_CHANNEL, SPELL_IMG, MAX_B_TN, IMDB, \
        ...
        try:
            if settings['auto_delete']:
                btn.insert(0, 
                    [
                    InlineKeyboardButton(f'ɪɴꜰᴏ', 'reqinfo'),
                    InlineKeyboardButton(f'ꜰᴏʀᴍᴀᴛ', 'sinfo')
                    ]
                )
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
                ...
                if clicked == typed:
                    if IS_VERIFY and not await check_verification(client, query.from_user.id):
                        ...
                        await client.send_message(
                            ...
                            text="<b>Yᴏᴜ ᴀʀᴇ ɴᴏᴛ ᴠᴇʀɪғɪᴇᴅ!\nKɪɴᴅʟʏ ᴠᴇʀɪғʏ ᴛᴏ ᴄᴏɴᴛɪɴᴜᴇ Sᴏ ᴛʜᴀᴛ ʏᴏᴜ ᴄᴀɴ ɢᴇᴛ ᴀᴄᴄᴇss ᴛᴏ ᴜɴʟɪᴍɪᴛᴇᴅ ꜱᴇʀɪᴇꜱ ᴜɴᴛɪʟ 12 ʜᴏᴜʀs ғʀᴏᴍ ɴᴏᴡ !</b>",
                        ...
                        await client.send_cached_media(
                            ...
                            reply_markup=InlineKeyboardMarkup(
                                [
                                [
                                InlineKeyboardButton('Uᴘᴅᴀᴛᴇs Cʜᴀɴɴᴇʟ', url="https://t.me/TEAM_COLD")
                                ]
                                ]
                            )
                        )
        ...
        if query.from_user.id in ADMINS:
            ...
            try:
                await client.send_message(chat_id=int(from_user), text=f"<b>Hey {user.mention}, Your Requested Series Has been Uploaded Now.Click Below Button And Send Requested Series Name in Our Group You Will Get The Series.\n\nതാങ്കൾ അഭ്യർത്ഥിച്ച സീരീസ് ആഡ് ചെയ്തിട്ടുണ്ട്.\nതാഴെക്കാണുന്ന ബട്ടൺ ക്ലിക്ക് ചെയ്ത് ഗ്രൂപ്പിൽ താങ്കൾ ആവശ്യപ്പെട്ട സീരിസിന്റെ പേര് അയച്ചാൽ സീരീസ് ലഭിക്കുന്നതാണ്.</b>", reply_markup=InlineKeyboardMarkup(btn3))
            except UserIsBlocked:
                await client.send_message(chat_id=int(SUPPORT_CHAT_ID), text=f"<b>Hey {user.mention}, Your Requested Series Has been Uploaded.Click Below Button And And Send You're Requested Series Name in Our Group You Will Get The Series.\nClick This Link Next Time You Will Get This Messege In Bot Dm.\nhttps://t.me/Cx_Series_BoT?start\nതാങ്കൾ അഭ്യർത്ഥിച്ച സീരീസ് ആഡ് ചെയ്തിട്ടുണ്ട് താഴെക്കാണുന്ന ബട്ടൺ ക്ലിക്ക് ചെയ്ത് ഗ്രൂപ്പിൽ സീരിസിന്റെ പേര് അയച്ചാൽ നിങ്ങൾ ആവശ്യപ്പെട്ട സീരീസ് ലഭിക്കുന്നതാണ്.മുകളിൽ കാണുന്ന ലിങ്ക് ക്ലിക്ക് ചെയ്ത് ബോട്ട് സ്റ്റാർട്ട്‌ ചെയ്താൽ അടുത്ത തവണ ഈ മെസ്സേജ് ബോട്ടിന്റെ pm ൽ ലഭിക്കുന്നതാണ്.</b>", reply_markup=InlineKeyboardMarkup(btn3))
        else:
            await query.answer("Yᴏᴜ ᴅᴏɴ'ᴛ ʜᴀᴠᴇ sᴜғғɪᴄɪᴀɴᴛ ʀɪɢʜᴛs ᴛᴏ ᴅᴏ ᴛʜɪs !", show_alert=True)
        ...
        if query.from_user.id in ADMINS:
            ...
            try:
                await client.send_message(chat_id=int(from_user), text=f"<b>Hey {user.mention}, Your Requested Series Already Available on Our Group.Click Below Button And Send Your Series Name In Our Group You Will Get The Series.\n\n താങ്കൾ ആവശ്യപ്പെട്ട സീരീസ് ഞങ്ങളുടെ ഗ്രൂപ്പിൽ ഉണ്ട്.താഴെക്കാണുന്ന ബട്ടൺ ക്ലിക്ക് ചെയ്ത് ഗ്രൂപ്പിൽ സീരിസിന്റെ പേര് അയച്ചാൽ സീരീസ് ലഭിക്കുന്നതാണ്.</b>", reply_markup=InlineKeyboardMarkup(btn3))
            except UserIsBlocked:
                await client.send_message(chat_id=int(SUPPORT_CHAT_ID), text=f"<b>Hey {user.mention}, Your Requested Series Already Available on Our Group.Click Below Button And Send series name In Our Group You Will Get The Series.\nClick This Link Next Time You Will Get This Messege In Bot Dm.\nhttps://t.me/Cx_Series_BoT?start\n താങ്കൾ ആവശ്യപ്പെട്ട സീരീസ് ഞങ്ങളുടെ ഗ്രൂപ്പിൽ ഉണ്ട്. താഴെക്കാണുന്ന ബട്ടൺ ക്ലിക്ക് ചെയ്ത് ഗ്രൂപ്പിൽ സീരിസിന്റെ പേര് അയച്ചാൽ താങ്കൾ ആവശ്യപ്പെട്ട സീരീസ് ലഭിക്കുന്നതാണ്.മുകളിൽ കാണുന്ന ലിങ്ക് ക്ലിക്ക് ചെയ്ത് ബോട്ട് സ്റ്റാർട്ട്‌ ചെയ്താൽ അടുത്ത തവണ ഈ മെസ്സേജ് ബോട്ടിന്റെ pm ൽ ലഭിക്കുന്നതാണ്.</b>", reply_markup=InlineKeyboardMarkup(btn3))
        else:
            await query.answer("Yᴏᴜ ᴅᴏɴ'ᴛ ʜᴀᴠᴇ sᴜғғɪᴄɪᴀɴᴛ ʀɪɢʜᴛs ᴛᴏ ᴅᴏ ᴛʜɪs !", show_alert=True)
        ...
        buttons = [[
                    ...
                    InlineKeyboardButton('🦋 𝙼𝙰𝙸𝙽 𝙲𝙷𝙰𝙽𝙽𝙴𝙻 🦋', url="https://t.me/TEAM_COLD")
                  ]]
        ...
        buttons = [[
            ...
            InlineKeyboardButton('Gʟᴏʙᴀʟ Fɪʟᴛᴇʀ', callback_data='global_filters')
        ],[
        ...
        await query.message.edit_text(
            ...
            btn = [[
                    ...
                    InlineKeyboardButton("Cᴏɴᴛᴀᴄᴛ", url="t.me/TEAM_COLD_BOT")
                  ]]
            await client.edit_message_media(
                query.message.chat.id, 
                query.message.id, 
...
async def auto_filter(client, msg, spoll=False):
    ...
    if ENABLE_SHORTLINK and settings["button"]:
        btn = [
            [
                InlineKeyboardButton(
                    text=f"🦋{file.file_name}", url=await get_shortlink(message.chat.id, f"https://telegram.me/{temp.U_NAME}?start=files_{file.file_id}")
                ),
            ]
            for file in files
        ]
    ...
    if imdb:
        ...
        cap = f"<b>々 ᴛɪᴛʟᴇ : {search}\n々 ʀᴇqᴜᴇꜱᴛᴇᴅ ʙy : {message.from_user.mention}\n々 ᴛᴏᴛᴀʟ ꜰɪʟᴇꜱ : {total_results}\n々 ᴘᴏᴡᴇʀᴇᴅ ʙʏ : {message.chat.title}\n\n⚠️ ᴛʜɪꜱ ᴍᴇꜱꜱᴀɢᴇ ᴡɪʟʟ ʙᴇ ᴀᴜᴛᴏ ᴅᴇʟᴇᴛᴇᴅ ᴀꜰᴛᴇʀ 10 ᴍɪɴᴜᴛᴇꜱ.\n</b>"
